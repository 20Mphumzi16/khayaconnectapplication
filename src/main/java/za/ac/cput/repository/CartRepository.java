...
public interface CartRepository extends JpaRepository<Cart,Long> {
    ...
    @Query("SELECT SUM(cb.price) FROM Cart c JOIN c.products cb WHERE c.cartId = :cartId")
    Double calculateCartTotalPrice(@Param("cartId") Long cartId);

    @Query("SELECT COUNT(cb) FROM Cart c JOIN c.products cb WHERE c.cartId = :cartId")
